...
void ParallelMultiexpTree<Curve>::initAccs() {
...
template <typename Curve>
uint32_t ParallelMultiexpTree<Curve>::getChunk(uint32_t scalarIdx, uint32_t chunkIdx) {
...
template <typename Curve>
void ParallelMultiexpTree<Curve>::processChunk(uint32_t idChunk) {
    for(uint32_t i=0; i<n; i++) {
        if (g.isZero(bases[i])) continue;
        uint32_t chunkValue = getChunk(i, idChunk);
        if (chunkValue) {
            __G_ADD__TREE__(accs[chunkValue].p, accs[chunkValue].p, bases[i]);
        }
    }
}

template <typename Curve>
void ParallelMultiexpTree<Curve>::reduce(typename Curve::Point &res, uint32_t nBits) 
{
    uint32_t ndiv2;
    ...
    while (true) {
        ...
        for (uint32_t i = 1; i<ndiv2; i++) {
            if (!g.isZero(accs[ndiv2 + i].p)) {
                __G_ADD__TREE__(accs[i].p, accs[i].p, accs[ndiv2 + i].p);
                __G_ADD__TREE__(accs[ndiv2].p, accs[ndiv2].p, accs[ndiv2 + i].p);
                g.copy(accs[ndiv2 + i].p, g.zero());
            }
        }
        for (u_int32_t i=0; i<nBits-1; i++) g.dbl(accs[ndiv2].p, accs[ndiv2].p);
        __G_ADD__TREE__(res, res, accs[ndiv2].p);
...
template <typename Curve>
void ParallelMultiexpTree<Curve>::multiexp(typename Curve::Point &r, typename Curve::PointAffine *_bases, uint8_t* _scalars, uint32_t _scalarSize, uint32_t _n, uint32_t _nThreads) {
...
/*        for (int index = 0; index < n; ++index) {
    ...
    for  (int j=nChunks-2; j>=0; j--) {
        ...
        __G_ADD__TREE__(r, r, chunkResults[j]);
    }
